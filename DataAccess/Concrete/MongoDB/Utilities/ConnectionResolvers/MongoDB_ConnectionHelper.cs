...
namespace DataAccess.Concrete.Databases.MongoDB.Utilities.ConnectionResolvers
{
    public class MongoDB_ConnectionHelper : IDatabase_ConnectionHelper
    {
        ...
        public IDataResult<DatabaseConnectionSettings> CheckDatabaseConnection()
        {
            ...
            if (result)
            {
                return new SuccessDataResult<DatabaseConnectionSettings>(new DatabaseConnectionSettings { HostName = $"mongodb://Oriana:{Statistics}@" + EnviromentVariable.DatabaseHostName, Database = databaseConnectionSettings.Database });
            }
